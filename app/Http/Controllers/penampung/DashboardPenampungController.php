...
class DashboardPenampungController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['user'] = Auth::user();
        $id_penampung = Auth::guard('penampung')->user()->id;

        $data['now'] = date('Y');
        $data['start'] = date('Y') - 5;
        $data['end'] = date('Y') + 5;
        $data['total_transaksi_donasi'] = DB::table('transaksi')->where('transaction_status', 'settlement')->sum('gross_amount');
        $data['total_pengeluaran_penampung'] = DB::table('transaksi_pengeluaran_penampung')->where('id_penampung', $id_penampung)->sum('total');
        $data['total_pengeluaran_admin'] = DB::table('transaksi_pengeluaran_donasi')->where('id_penampung', $id_penampung)->sum('total');
    ...
    public function total_lapor()
    {
        ...
        $seriesData['showInLegend'] = false;
        foreach($tempCategory as $val){
            $data = DB::table('laporan')->where('id_penampung', $id_penampung)->whereYear('created_at', $tahun)->whereMonth('created_at', $val)->count();
            $seriesData['name'] = 'Total';
            $seriesData['data'][] = (int)$data;
        }
        $series = $seriesData;
